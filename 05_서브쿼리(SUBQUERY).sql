...
/*
    *서브쿼리의 구분
    ...
    --다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한행이지만 컬럼이 여러개 일때
    --다중행 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 여러행 여러 컬럼일때
...
--2-2. 대리 직급 이면서 위의 결과값보다 하나라도 큰 사원
SELECT
    EMP_ID,
    EMP_NAME,
    JOB_NAME,
...
--2) 박나라 사원과 같은 직급코드, 같은 사수를 가지고 있는 사원들의 사번, 사원명, 직급코드, 사수번호
SELECT
    EMP_ID,
    EMP_NAME,
    JOB_CODE,
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) > (SELECT SUM(SUM(SALARY))
                      FROM EMPLOYEE
                      GROUP BY SALARY) * 0.2;
...
-------------------------------------------------------------------------------
/*
    5. 인라인 뷰 (INLINE VIEW ? 이거맞나)
    FROM절에 서브쿼리를 작성하는 것
    서브쿼리를 수행한 결과를 마치 테이블 처럼 사용
*/

--1) 사원들의 사번, 이름 ,보너스포함 연봉, 부서코드 조회
--단, 보너스 포함 연봉은 NULL이면 안된다
--단, 보너스 포함 연봉이 30000000만원 이상인 사원들만 조회
...
-->인라인뷰를 주로 사용하는 예 >> TOP-N분석 : 상위 N개만 조회
...
SELECT
    ...
    DEPT_CODE,
    평균급여
FROM(SELECT DEPT_CODE, AVG(SALARY) AS "평균급여" FROM EMPLOYEE GROUP BY DEPT_CODE ORDER BY AVG(SALARY) DESC)
WHERE ROWNUM <= 3;
...
/*
    *순위를 매기는 함수(WINDOW FUNCTION)
    ...
    RANK() OVER(정렬기준) : 동일한 순위 이후와 등수를 동일한 인원 수 만큼 건너뛰고 순위계산
    DANSE_RANK() OVER : 동일한 순위가 있다고 해도 그다음 등수를 무조건 1씩 증가
