...
ORDER BY DEPT_CODE; --4 ORDER BY는 무조건 마지막 실행 -> 정렬ㄹ은 마지막 해주는 것이 아니면 의미X

--각 직급별 총사원수, 보너스를 받는 사람수, 급여합, 평균급여, 최저급여, 최고급여 (정렬 = 직급 오름차순)
SELECT JOB_CODE, 
    COUNT(*) AS "사원수", 
    COUNT(BONUS)AS "보너스를 받는 사람수", 
    SUM(SALARY)AS "급여합",
...
--각 부서별 평균급여가 300만원인 이상인 부서들만 조회(부서코드 평균급여)
...
--직급별 직급코드, 총급여함(단, 직급별 급여합이 1000만원 이상인 직급만 조회)
...
/*
    ...
    -UNION ALL : 합집합 + 교집합(중복되는 거 상관않고 전부 더하는 것)
...
WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_ID, EMP_NAME,DEPT_CODE,SALARY --SALARY 자리에 다른게 들어가면 안된다
FROM EMPLOYEE
WHERE SALARY > 3000000
ORDER BY EMP_ID;
--컬럼의 개수가 동일해야 한다.
--컬럼 자리마다 동일한 타입으로 기술해야 한다.
--타입이 같으면 실행은되나 예외가 발생한다.
...
--3. MINUS : 선행 SELECT 결과에 후행 SELECT결과를 뺀 나머지
--부서코드가 D5인 사람들중에 급여가 300만원을 초과하지않는 사원
SELECT EMP_ID, EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
