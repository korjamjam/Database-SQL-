...
/*
    ...
    -DESC : 내림차순(큰 값으로 시작해서 값이 점점 작아지는 것)
...
SELECT EMP_NAME AS 사원명, SUBSTR(EMP_NO,1 ,8) || 'XXXXXX' AS 주민등록번호
--SELECT EMP_NAME, RPAD(SUBSTR(EMP_NO, 1, 8), 14, 'X') AS 주민등록번호
FROM EMPLOYEE;
...
SELECT EMP_NAME AS 사원명, HIRE_DATE AS 입사일, ADD_MONTHS(HIRE_DATE, 3) AS 입사후3개월뒤
FROM EMPLOYEE;
...
SELECT TO_CHAR(12, '99999') FROM DUAL; --9의 자리수 만큼 공간 확보, 오른쪽 정렬
SELECT TO_CHAR(12, '00000') FROM DUAL; --0의 자리수 만큼 공간 확보, 빈칸을 0으로 채움
SELECT TO_CHAR(2000000, 'L99999') FROM DUAL; -- 현재 설정된 나라의 로컬 화폐단위로 나타남.
...
SELECT TO_DATE(240807) FROM DUAL; --50년 미만은 자동으로 20XX으로 설정된다 50년 이상은 19XX으로 설정된다.

SELECT TO_DATE(001207) FROM DUAL; -- 숫자는 0으로 시작할 수 없다.
SELECT TO_DATE('001207') FROM DUAL; --문자로 쓰면 오류없이 잘 된다.

SELECT TO_DATE('001207 120500') FROM DUAL; -- 포멧을 정해줘야 시, 분, 초를 표시할 수 있다.
SELECT TO_DATE('001207 120500', 'YYMMDD HH24MISS') FROM DUAL;

--===========================================================================
/*
    TO_SUMBER : 문자타입의 데이터를 숫자타입으로 변환시켜주는 함수
...
/*
    [선택함수]
    *DECODE(비교하고자하는 대상(컬럼, 연산식, 함수식...등), 비교값1, 결과값1, 비교값2, 결과값2...)
*/

--사번, 사원명, 주민번호, 성별 조회
SELECT  EMP_ID, EMP_NAME, EMP_NO,
        DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여', '3', '남', '3', '여', '외계인') AS 성별
FROM EMPLOYEE;

--직원의 사원명, 기존 급여, 인상된 급여 조회 * 각 직급별로 인상해서 조회
--직급이 J7인 사원은 급여를 10%인상 (SALARY * 1.1)
--J6인 사원은 급여를 15%인상
--J5인 사원은 급여를 20%인상
--그외 사원들은 급여를 5%인상

SELECT EMP_NAME, SALARY AS "인상전 급여",
DECODE(JOB_CODE, 'J7', SALARY * 1.1,'J6', SALARY * 1.15,'J5', SALARY * 1.2, SALARY *1.05) AS "인상후 급여"
FROM EMPLOYEE;
...
--3. MAX(모든타입가능) : 해당 컬럼값 중 가장 큰 값을 구해서 반환
